...
Firoku = {
    ...
    do_roku_button: function (act, button) {
        ...
        $.ajax(url).fail(function (jqxhr, status, error) {
            Firoku.do_error("Error communicating with Roku (" + status + ")");
        });
    },
    ...
    load_channels: function() {
        ...
        a.done(function(d, s, j) { Firoku.display_channels(d, s, j); });
        a.fail(function (jqxhr, status, error) {
            Firoku.do_error("Error retrieving channel list (" + status + ")");
        });
    },
    ...
    launch_channel: function(chanid) {
        ...
        $.ajax(url).fail(function (jqxhr, status, error) {
            Firoku.do_error("Error launching channel (" + status + ")");
        });
    },
...
(function ($) {
    ...
    for (var b of roku_buttons) {
        (function(btn) {
            for (var d of Firoku.downevents) {
                $("#b_" + btn).on(d, function () {
                    Firoku.do_touch_start(btn);
                    Firoku.do_roku_down(btn);
                });
            }
            for (var u of Firoku.upevents) {
                $("#b_" + btn).on(u, function () {
                    Firoku.do_touch_stop(btn);
                    Firoku.do_roku_up(btn);
                });
            }
        })(b);
    }
    ...
    for (var d of Firoku.downevents) {
        $("#b_settings").on(d, function () {
            Firoku.do_touch_start("settings");
        });

        $("#b_remote").on(d, function () {
            Firoku.do_touch_start("remote");
        });

        $("#b_channels").on(d, function() {
            Firoku.do_touch_start("channels");
        });
    }

    for (var u of Firoku.upevents) {
        $("#b_settings").on(u, function () {
            Firoku.do_touch_stop("settings");
            Firoku.show_panel("settings");
        });

        $("#b_remote").on(u, function () {
            Firoku.do_touch_stop("remote");
            Firoku.show_panel("remote");
        });

        $("#b_channels").on(u, function () {
            Firoku.do_touch_stop("channels");
            Firoku.show_panel("channels");
            Firoku.load_channels();
        });
    }

    $("#roku_ip").on('blur', function () { Firoku.save_ip(); });
